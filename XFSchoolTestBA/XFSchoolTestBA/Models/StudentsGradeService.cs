...
namespace XFSchoolTestBA.Models
{
    class StudentsGradeService
    {
            ...
            const string Url = "http://pakdmitriy1989-001-site1.htempurl.com/api/StudentsGrade";
            ...
            public async Task<StudentsGrade> Add(StudentsGrade studentsGrade)
            {
                HttpClient client = GetClient();
                var response = await client.PostAsync(Url,
                    new StringContent(
                        JsonConvert.SerializeObject(studentsGrade),
                        Encoding.UTF8, "application/json"));

                if (response.StatusCode != System.Net.HttpStatusCode.OK)
                    return null;

                return JsonConvert.DeserializeObject<StudentsGrade>(
